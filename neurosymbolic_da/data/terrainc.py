...
TERRA_INC_DOMAINS = ("location_38", "location_43", "location_46", "location_100")
...
def _load_split_indices(root: str, domain: str, split: str = "train") -> list[int]:
    ...
    Args:
        root: path to terra directory containing split.json
...
def get_terra(
    ...
    if not domain_path.exists():
        raise FileNotFoundError(
            f"Terra domain folder not found at {domain_path}. "
        )
...
def get_terra_loaders(
    ...
    """Get source/target train/test loaders for a terra transfer task.

    Returns:
        (source_train, source_test, target_train, target_test)
    """
    src_train = get_terra(root, source, train=True, image_size=image_size,
                          strong_aug=strong_aug, randaugment=randaugment)
    src_test = get_terra(root, source, train=False, image_size=image_size)
    tgt_train = get_terra(root, target, train=True, image_size=image_size,
                          strong_aug=strong_aug, randaugment=randaugment)
    tgt_test = get_terra(root, target, train=False, image_size=image_size)
