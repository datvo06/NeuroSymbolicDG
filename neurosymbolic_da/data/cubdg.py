"""CUB-DG dataset (Caltech-UCSD Birds with Domain Generalization splits).

4 domains: Photo, Art, Cartoon, Paint — 200 bird species, 11788 images per domain.
Uses official train/test split from split.json (image indices shared across domains).

Expected layout: root/CUB-DG/{Photo,Art,Cartoon,Paint}/001.Black_footed_Albatross/*.jpg
"""
...
CUBDG_DOMAINS = ("Photo", "Art", "Cartoon", "Paint")
...
def _load_split_indices(root: str, domain: str, split: str = "train") -> list[int]:
    ...
    Args:
        root: path to CUB-DG directory containing split.json
...
def get_cubdg(
    ...
    """Load a CUB-DG domain with official train/test split.

    Args:
        root: path to CUB-DG directory
        domain: one of "Photo", "Art", "Cartoon", "Paint"
    ...
    if not domain_path.exists():
        raise FileNotFoundError(
            f"CUB-DG domain not found at {domain_path}. "
            f"Download CUB-DG to {root}"
        )
    ...
    indices = _load_split_indices(root, domain, split)
    return Subset(full_dataset, indices)


def get_cubdg_loaders(
    root: str,
    source: str,
    target: str,
    batch_size: int = 32,
    ...
    """Get source/target train/test loaders for a CUB-DG transfer task.

    Returns:
        (source_train, source_test, target_train, target_test)
    """
    src_train = get_cubdg(root, source, train=True, image_size=image_size,
                          strong_aug=strong_aug, randaugment=randaugment)
    src_test = get_cubdg(root, source, train=False, image_size=image_size)
    tgt_train = get_cubdg(root, target, train=True, image_size=image_size,
                          strong_aug=strong_aug, randaugment=randaugment)
    tgt_test = get_cubdg(root, target, train=False, image_size=image_size)
