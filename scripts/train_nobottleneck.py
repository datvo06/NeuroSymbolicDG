#!/usr/bin/env python3
"""NoBottleneck ablation training script (Experiment 4b, Section 6.6).

Trains backbone + projection + PCFG (no concept bottleneck heatmaps).
Isolates the contribution of the spatial concept bottleneck.

Usage:
    uv run python scripts/train_nobottleneck.py \
...
def main():
    parser = argparse.ArgumentParser(
        description="NoBottleneck ablation training (Phase 1)"
    )
    parser.add_argument(
    ...
    print("Pipeline: NoBottleneck (backbone + projection + PCFG)")

    model = NoBottleneckPipeline(
    ...
    save_path = (
        args.save_path
        or f"checkpoint_nobn_{args.dataset}_{args.source}_{args.target}.pt"
    )
    metrics = train(
        model=model,
