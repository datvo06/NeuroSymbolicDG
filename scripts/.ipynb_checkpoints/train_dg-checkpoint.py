...
DATASET_DOMAINS = {
    ...
    "vlcs": ["photo", "art_painting", "cartoon", "sketch"],
}
...
def get_dg_loaders(
    ...
    if dataset == "cubdg":
        ...
        from neurosymbolic_da.data.office import get_officehome

        src_train_datasets = []
        src_val_datasets = []
        for domain in source_domains:
            src_train_datasets.append(
                get_officehome(data_root, domain, train=True, image_size=image_size,
                         strong_aug=strong_aug, randaugment=randaugment)
            )
            src_val_datasets.append(
                get_officehome(data_root, domain, train=False, image_size=image_size)
            )

        combined_train = ConcatDataset(src_train_datasets)
        combined_val = ConcatDataset(src_val_datasets)

        tgt_test = get_officehome(data_root, target, train=False, image_size=image_size)
...
def get_dg_per_domain_loaders(
    ...
    if dataset == "cubdg":
        ...
        val_datasets = []
        for domain in source_domains:
            train_ds = get_officehome(data_root, domain, train=True,
                                image_size=image_size, strong_aug=strong_aug,
                                randaugment=randaugment)
            val_ds = get_officehome(data_root, domain, train=False,
                              image_size=image_size)
            kwargs = dict(batch_size=batch_size, num_workers=num_workers,
                          pin_memory=True)
            domain_train_loaders.append(
